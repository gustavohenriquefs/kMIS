...
std::vector<VNSReport> VNS_Reativo(Graph& graph, Solucao& solucao_entrada, std::chrono::high_resolution_clock::time_point start_time) {
  ...
  Grasp grasp;
  Solucao solucao = solucao_entrada;            // best solution found
  Solucao solucao_candidata = solucao_entrada;  // current solution being explored
  ...
  float tempo_limite_ms = 40000.0;  // 40 segundos
  
  for (int i = 0; tempo_limite_ms >= TIME_DIFF(start_time, get_current_time()); i++) {
    q = 1;
    qtd_sem_melhorar = 0;
    qtd_igual = 0;
    while (q <= q_max) {
      ...
      Solucao solucao_linha = SO_Reativo(graph, solucao_candidata, q, alfa);  // Solution after shaking
